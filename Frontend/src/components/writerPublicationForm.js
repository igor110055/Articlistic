...
const WriterPublicationForm = ({closeModal}) => { 
  ...
  const {userUserName } = useSelector(
    (state) => ({
  ...
  const createPublication = async (e) => {
    ...
    if(publicationName.trim().length < 3 || publicationName.trim().length > 30) {
    ...
    dispatch(createPublications({token, fd, userUserName, name: publicationName.trim()}))
    setIsCreatingPublication(false)
    closeModal()
    ...
    <div className={classes.publishForm}>
      ...
      <Box marginTop="15px">
        <Button
          ...
          >          
          {isCreatingPublication && <CircularProgress size={20} style={{ color: 'white' }} />}
