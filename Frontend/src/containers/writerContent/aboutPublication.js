import { Button, Container, Paper, Tab, Tabs, Typography } from "@mui/material";
...
const AboutPublication = () => {
  ...
  const getWriterName = async () => {
    const response = await axios.get(`${baseURL}/users/name?username=${userDetails?.userUserName}`)
  ...
  return (
    <div>
      <div className={classes.saveBar} id="#e">
        ...
        <div className={classes.writerNameContainer}>
          <div className={classes.writerName}><span className={classes.span}>By </span>{writerName} </div>
        </div> 
      ...
      <div className={classes.container}>
        <Box className={classes.TabContainer}>
           ...
           <TabPanel value={value} index={0}>        
              <TopStoriesPublication writerName={writerName} publication={publication} />            
