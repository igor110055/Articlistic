...
const ImageUpload = ({ onDataChange, data, readOnly }) => {
    ...
    const fileUploadHandler = (e) => {
        ...
        // console.log(temp);
        axios.post(`${baseURL}/${endPoints.publicationUploadImage}?username=${user.userUserName}&publicationId=${publicationId}`, fd, { //for writers
            headers: {
                "Authorization": temp,
            },
    ...
    return (
        <div className={classes.imageUploadSection}>
            {url === '' ? (< div >
                ...
                {progress === 0 ? (<div className={classes.input} onClick={handleInputClicked}>Select Image</div>)
                    : (
                        <div className={classes.progress}>{progress}% Uploaded</div>
                    )}
            </div>) : < div >
                < img src={url} className={classes.imageStyle} />
                {!readOnly && <input className={classes.captionStyle} value={caption} onChange={(e) => {
