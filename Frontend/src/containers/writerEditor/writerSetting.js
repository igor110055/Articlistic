...
import { Checkbox, Button, CircularProgress, RadioGroup, FormControlLabel, Radio, Alert } from '@mui/material';
...
const WriterSetting = () => {
        ...
        if (getArticleError) {
            ...
            if (!isGettingArticle && getArticleInitiate) {
                if (getArticleResp.article.status !== 'DRAFT') {
                    let temp = [];
                    for (let i = 0; i < getArticleResp.article.storySetting.categories.length; i++) {
                        temp.push({ name: getArticleResp.article.storySetting.categories[i] });
                    }
                    // console.log(temp);
                    setTags(temp)
                }
    ...
    useEffect(() => {
        ...
        dispatch(getPublications({token, userUserName: JSON.parse(localStorage.getItem('user')).userUserName}))
    }, []);
    ...
    useEffect(() => {
        if (userUserName !== '') {
            ...
            dispatch(getArticle({ articleId: localStorage.getItem('articleId'), username: userUserName }, temp));
        ...
        <div>
            ...
            <div className={classes.saveBar}>
                ...
                style={{ marginRight: '0.5%', width: '8rem' }}
                    ...
                    onClick={publish}
                >
                    {isUploadingArticle ? <CircularProgress size={20} style={{ color: 'white' }} /> : getArticleResp?.article?.status === 'PUBLISHED' ? `Republish` : `Publish`}
                </Button>
            </div> 
          
            <div className={classes.settingContainer}>
                <div className={classes.settingContainerWrap}>
                    <div className={classes.writerSettinggTitle}>Story Setting</div>
                    <div className={classes.writerSettingCard}>
                        <div className={classes.cardTitle}>Reader Interests</div>
                        {tagValidation && <div className={classes.tagValidation}> {tagValidation} </div> }
                        {noCategoriesFound && <div className={classes.disabledPublicationMsg}>
                            Please add atleast one tag. </div>}
                        <div>
                            <ReactTags
                                ref={reactTags}
                                tags={tags}
                                ...
                                onAddition={(e) =>{
                                    ...
                                    if(!/^[a-zA-Z]*$/g.test(e.name)){
                                        setTagValidation("Tags shouldn't include symbols or numbers")
                                        return
                                    }
                ...
                <div className={classes.settingContainerWrap}>
                    <div className={classes.writerSettinggTitle}>Publication Settings</div>
                    <div className={classes.writerSettingCard}>
                        <div className={classes.cardTitle}>Select a Publication</div>
                        {localStorage.getItem('articleStatus') === 'PUBLISHED' && <div className={classes.disabledPublicationMsg}> Publication cannot be changed for already published story. </div> }
                       
                        {publicationErrorMsg && <Alert style={{margin: '10px'}} severity="error">{publicationErrorMsg}</Alert>}
                        <RadioGroup
                        ...
                        >
                           {publicationsData.map((publication) => 
                                <FormControlLabel
                                    disabled={localStorage.getItem('articleStatus') === 'PUBLISHED'}
                                ...
                                label={<span className={classes.publicationRadioBtns}>{publication.publicationName}</span>} />
                           )}
                ...
                <div className={classes.settingContainerWrap}>
                    <div className={classes.writerSettinggTitle}>Publish Setting</div>
                    <div className={classes.writerSettingCard}>
                        {/* <div className={classes.cardTitle}>Get early access</div> */}
                        <div>
                            <Checkbox checked={true} disabled lable="Early Access" onChange={handleCheck} />
