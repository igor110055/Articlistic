...
const WritersAndCategories = () => {
  ...
  const {
    ...
    return {
      ...
      user: state.user,
      isSendingWritersAndCategories:
        state.writers.isSendingWritersAndCategories,
  ...
  const handleFinish = () => {
    if (
      Object.keys(selectedWriters).length >= 5 &&
  ...
  return isGettingWriters ? (
    ...
    <div>
      ...
      <div className="writer-selection">
        <div className="select-title">
          ...
          <div className="writer-title">Select Writer</div>
        </div>
    ...
    <div className="api-error">{getWritersErrorMsg}</div>
  );
};
